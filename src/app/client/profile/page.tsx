  ...
  return (
    <div className="flex-1 w-full max-w-md mx-auto px-4 py-6 flex flex-col justify-between min-h-screen text-primary">
      <div>
        ...
        {/* 1. SEARCH BOX */}
        <div className="vintage-panel p-4 mb-6">
          <form onSubmit={handleLookupSubmit} className="space-y-3">
            <label className="text-[9px] font-medium text-muted-foreground uppercase tracking-wider block">
          ...
          {errorMsg && (
            <div className="flex items-center gap-2 text-xs text-destructive bg-destructive/10 border border-destructive/20 p-2.5 rounded mt-3">
        ...
        {searched && client && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="vintage-frame rounded shadow-xl space-y-4">
              <div className="flex justify-between items-start border-b border-[#d4af37]/20 pb-3">
                <div>
                  <h2 className="text-lg font-sans font-bold text-foreground uppercase">{client.name}</h2>
                  <p className="text-[10px] text-muted-foreground flex items-center gap-1 mt-0.5">
                    <Phone className="h-3 w-3 text-primary" /> {client.phone}
                  </p>
                </div>
                <div className="text-right">
                  <span className="text-[9px] text-muted-foreground block uppercase">Visitas Totales</span>
                  <span className="text-base font-sans font-bold text-primary">{client.totalVisits}</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider flex items-center gap-1.5">
                    <Award className="h-4 w-4 text-primary" /> Tarjeta de Fidelización
                  </span>
                ...
                {/* Stamp Board */}
                <div className="bg-card/80 p-4 rounded border-2 border-primary/20 flex flex-wrap justify-center gap-3">
                ...
                <div className="text-center pt-2">
                  ...
                  {client.loyaltyRedeemed > 0 && (
                    <p className="text-[9px] text-success mt-1.5 font-semibold">
      ...
      <footer className="mt-8 pt-4 border-t border-primary/20 flex flex-col items-center gap-2">
        <p className="text-[9px] text-primary/70 uppercase tracking-wider font-bold">
          © {new Date().getFullYear()} {barberName} • Juan Rairan
        </p>
      </footer>
    </div>
  );
